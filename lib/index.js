...
function stringifyObject(object) {
  let keys = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Object.keys(object).concat(Object.getOwnPropertySymbols(object));
...
function getCacheKey(model, attribute, options) {
  // TODO: better way?? related to changes in graphql-sequelize so that findOptions have requestUser
  let optionsKey = stringifyObject(options, ['association', 'attributes', 'groupedLimit', 'limit', 'offset', 'order', 'where', 'through', 'raw', 'requestUser']);
  ...
  _shimmer2.default.wrap(target, 'get', original => {
    return function bathedGetHasMany(instances) {
      ...
      if (this.through.scope) {
        ...
        options.through.where = {
          $and: [options.through.where, scope]
        };
      }
